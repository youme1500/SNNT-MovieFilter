...
from mks.database.pm_database import Media, get_file_details, unpack_new_file_id, get_bad_files, parse_link
...
from mks.utils.utils import get_settings, get_size, is_subscribed, save_group_settings, temp
from mks.database.connections_mdb import active_connection
import re
import json
import base64 
...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        if not await db.get_chat(message.chat.id):
            ...
            await client.send_message(Config.LOG_CHANNEL, Script.LOG_TEXT_G.format(message.chat.title, message.chat.id, total, "Unknown"))       
    ...
    if len(message.command) != 2:
        ...
        return
    if Config.AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        if not msgs:
            ...
            try: 
                ...
                return await client.send_message(Config.LOG_CHANNEL, "UNABLE TO OPEN FILE.")
            os.remove(file)
        ...
        for msg in msgs:
            ...
            if Config.BATCH_FILE_CAPTION:
                try:
                    f_caption=Config.BATCH_FILE_CAPTION.format(file_name= '' if title is None else title, file_size='' if size is None else size, file_caption='' if f_caption is None else f_caption)
            ...
            try:
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup( [ [ InlineKeyboardButton('⚔️ 𝖴𝖯𝖣𝖠𝖳𝖤𝖲 ⚔️', url=f"{Script.ALL_LINK}") ] ] ),
        ...
        try:
            ...
            protect = "/pbatch" if Config.PROTECT_CONTENT else "batch"
        ...
        async for msg in client.iter_messages(int(f_chat_id), int(l_msg_id), int(f_msg_id)):
            if msg.media:
                ...
                if Config.BATCH_FILE_CAPTION:
                    try:
                        f_caption=Config.BATCH_FILE_CAPTION.format(file_name=getattr(media, 'file_name', ''), file_size=getattr(media, 'file_size', ''), file_caption=getattr(msg, 'caption', ''))
                    except Exception as e:
                        logger.exception(e)
    ...
    if not files_:
        ...
        try:
            ...
            if Config.CUSTOM_FILE_CAPTION:
                try:
                    f_caption=Config.CUSTOM_FILE_CAPTION.format(file_name= '' if title is None else title, file_size='' if size is None else size, file_caption='')
                except:
                    return
...
@Client.on_message(filters.command('channel') & filters.user(Config.ADMINS))
async def channel_info(bot, message):
           
    """Send basic information of channel"""
    if isinstance(Config.CHANNELS, (int, str)):
...
@Client.on_message(filters.command('delete1') & filters.user(Config.ADMINS))
...
@Client.on_message(filters.command('delete2') & filters.user(Config.ADMINS))
async def pmdelete(bot, message):
    """Delete file from database"""
    reply = message.reply_to_message
    if reply and reply.media:
        msg = await message.reply("Processing...⏳", quote=True)
...
@Client.on_message(filters.command('deleteall') & filters.user(Config.ADMINS))
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text="BOT DM Data File DELETE ALL", callback_data="autofilter_delete1"
                    )
                ],
                [             
                    InlineKeyboardButton(
                        text="Channel Data File DELETE ALL", callback_data="autofilter_delete2"
                    )
                ],
                [
                    InlineKeyboardButton(
...
@Client.on_callback_query(filters.regex(r'^autofilter_delete1'))
...
async def pmdelete_all_index_confirm(bot, message):
    ...
    if settings is not None:
        buttons = [
            [
                ...
                InlineKeyboardButton(
                    '✅ 𝖮𝗇' if settings["button"] else '❌ 𝖮𝖿𝖿',
                    callback_data=f'setgs#button#{settings["button"]}#{grp_id}',
                ),
            ],
            [
                ...
                InlineKeyboardButton(
                    '✅ 𝖮𝗇' if settings["PM_SEND"] else '❌ 𝖮𝖿𝖿',
                    callback_data=f'setgs#PM_SEND#{settings["PM_SEND"]}#{str(grp_id)}'
                ),
            ],
            [
            ...
            [
                ...
                InlineKeyboardButton(
                    '10' if settings["max_btn"] else f'{Config.MAX_B_TN}',
                    callback_data=f'setgs#max_btn#{settings["max_btn"]}#{grp_id}',
                ),
            ],
        ]
...
async def requests(bot, message):
    if Config.REQST_CHANNEL is None or Config.SUPPORT_CHAT_ID is None: return # Must add REQST_CHANNEL and SUPPORT_CHAT_ID to use this feature
    if message.reply_to_message and Config.SUPPORT_CHAT_ID == message.chat.id:
        ...
        try:
            if Config.REQST_CHANNEL is not None:
                btn = [[
    ...
    elif Config.SUPPORT_CHAT_ID == message.chat.id:
        chat_id = message.chat.id
        reporter = str(message.from_user.id)
        mention = message.from_user.mention
        success = True
        ...
        try:
            if Config.REQST_CHANNEL is not None and len(content) >= 3:
                btn = [[
...
@Client.on_message(filters.command("usend") & filters.user(Config.ADMINS))
async def send_msg(bot, message):
    if message.reply_to_message:
...
@Client.on_message(filters.command("gsend") & filters.user(Config.ADMINS))
...
@Client.on_message(filters.command("deletefiles") & filters.user(Config.ADMINS))
